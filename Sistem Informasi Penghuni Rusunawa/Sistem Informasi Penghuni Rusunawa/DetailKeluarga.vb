...
Public Class DetailKeluarga
    ...
    Sub ketemu()
        Label8.Text = UCase(DR.Item("nama_kk"))
        Label9.Text = DR.Item("id_kk")
        Label10.Text = DR.Item("id_unit")
        Label11.Text = DR.Item("no_kwh")
        Label12.Text = "Jl. " + UCase(DR.Item("jalan")) + ", RT." + DR.Item("rt") + "/" + DR.Item("rw") + ", No." + DR.Item("no") + ", Kel. " + (UCase(DR.Item("kelurahan")) + ", Kec. " + UCase(DR.Item("kecamatan")) + ", Kota. " + UCase(DR.Item("kota")) + ", Provinsi. " + UCase(DR.Item("provinsi")) + " - " + DR.Item("kode_pos"))
        Label13.Text = "Gedung " + DR.Item("gedung") + " Lantai " + DR.Item("lantai") + " Kamar " + DR.Item("kamar") + " (" + DR.Item("unit") + ")"
    End Sub

    Sub tampilPendudukCari()
        koneksi()
        DA = New MySqlDataAdapter("SELECT tbl_penduduk.nik, tbl_penduduk.nama, tbl_penduduk.status_kk, tbl_penduduk.jenis_kelamin, tbl_penduduk.tmpt_lahir, tbl_penduduk.tgl_lahir, umur_p.t_lahir FROM tbl_penduduk, umur_p WHERE tbl_penduduk.nik = umur_p.nik AND tbl_penduduk.id_kk = '" & Label9.Text & "'", CONN)
        DS = New DataSet
        DA.Fill(DS)
